...
import { ICatalog, ICatalogRepo, PaginatedResponse } from "./repo";
...
export class CatalogService implements ICatalogService {
  constructor(private catalogRepo: ICatalogRepo, private redis: RedisClient
){}
 ...
 async getOne(id: string): Promise<ICatalog | null> {
  ...
  limit === undefined
    ...
    const data = await this.catalogRepo.findAll(page, limit)
     ...
     await this.redis.set(`${key}`, JSON.stringify(data), {expiration: {type: 'EX', value: 120}})
